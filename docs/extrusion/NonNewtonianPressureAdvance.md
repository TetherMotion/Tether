...
|-----------|------------------------------------------------------|-------------------------------------|
| `linear`  | `δe = PA · v_e`                                      | Newtonian fluids, classic Klipper PA |
| `power_law` | `δe = K_base · (v_e · A_f)^n`                      | Shear-thinning melts with known flow index |
| `cross_wlf` | `δe = (βV_m/A_f) · P_LUT(Q, T)`                   | Full temperature-dependent Cross-WLF rheology |
...
| `pa_consistency`                  | float  | `0.0`     | K_base [filament-mm / (mm³/s)^n]         |
| `pa_max_compensation`             | float  | `0.5`     | Safety clamp on \|δe\| [mm]              |
| `cross_wlf_tau_star`              | float  | `1e5`     | Cross τ* [Pa]                            |
| `cross_wlf_flow_index`            | float  | `0.4`     | Cross *n*                                |
| `cross_wlf_c1`                    | float  | `17.44`   | WLF C1                                   |
| `cross_wlf_c2`                    | float  | `51.6`    | WLF C2 [K]                               |
| `cross_wlf_ref_temp`              | float  | `200.0`   | WLF T_ref [°C]                           |
| `cross_wlf_zero_shear_viscosity`  | float  | `1000.0`  | η_ref [Pa·s]                             |
| `cross_wlf_compressibility_over_area` | float | `0.0`  | βV_m/A_f [mm/Pa]                         |
...
```
δe = K_base · Q^n = K_base · (v_e · A_f)^n
```

where:
- `K_base = βV_m·C_n / A_f` [filament-mm / (mm³/s)^n]
- `Q = v_e · A_f` is the volumetric flow [mm³/s]
- `n` is the flow index (n < 1 = shear-thinning, n = 1 = Newtonian)

When `n = 1` and `K_base = PA`, the power-law model reduces exactly to the
linear model.
...
```
η(T, γ̇) = η_0(T) / (1 + (η_0(T)·γ̇ / τ*)^(1-n))

η_0(T) = η_ref · C1·(T-T_ref) / (C2 + T-T_ref)   [WLF shift]
```
