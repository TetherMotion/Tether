...
class AnalogInputTerminal : public IAnalogInputTerminal {
public:
    ...
    /// Raw input process image as last exchanged (SM3 region).
    std::span<const uint8_t> rawInput() const { return in_buf_; }

    /// Raw output process image for terminals with an enabled SM2
    /// (e.g. thermocouple compensation words).  Not interpreted — write
    /// at your own risk; empty for pure input terminals.
...
private:
    ...
    /// Resolve process-data SM addresses/lengths/ctrl and the per-channel
    /// value/status offsets from the bound discovery data / SII.
    ...
    /// finalizeMapping().  Ends before SAFE-OP so the chain can interpose
    /// FMMU programming.
    ...
    /// Heap-allocated so the pointers stay valid when the driver object is
    /// moved (e.g. returned from findFirst()).
    std::vector<uint8_t> in_buf_;    ///< SM3 process image
    std::vector<uint8_t> out_buf_;   ///< SM2 scratch (only when SM2 enabled)

    std::vector<ChannelLayout> channels_;

    // Sync-manager resolution (SII)
    uint8_t  sm_in_channel_   = 3;
    uint16_t sm_in_addr_      = 0x1180;
    uint16_t sm_in_len_       = 0;
    uint8_t  sm_in_ctrl_      = 0x20;
    bool     sm_out_enabled_  = false;
