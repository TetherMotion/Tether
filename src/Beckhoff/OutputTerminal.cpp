...
Result<> OutputTerminal::prepare(PDO::PDOAddressMode mode) {
    ...
    int entry = pdo.mapping().add_rxpdo(
        slave_index_, sm_len_, rxpdo_index_, mode);
    if (entry < 0) {
        return std::unexpected(Error::PdoRegistrationFailed);
    }
    // Bind the state word into the entry's storage — writes made through
    // set()/setBit() go straight onto the wire on the next exchange.
