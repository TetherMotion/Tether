...
bool AS715NFaultHandler::checkFault(EtherCAT::CoE::CoEManager& sdo, uint16_t slave_idx, uint16_t* mfr_error, uint16_t* cia402_error) {
    ...
    if (!sw_result.has_value()) {
        ...
        if (mfr != 0 || cia != 0) {
            ...
            TETHER_LOGW(TAG, "Slave %u: also failed to read 0x203F/0x603F (mailbox/CoE may be unavailable yet)", slave_idx);
        }
    ...
    if (has_fault) {
        TETHER_LOGW(TAG, "Slave %u: Fault detected! StatusWord=0x%04X", slave_idx, statusword);
        ...
        if (statusword & (1u << 7)) {
            TETHER_LOGW(TAG, "Slave %u: Warning active (StatusWord=0x%04X, no fault)", slave_idx, statusword);
        }
    }
...
bool AS715NFaultHandler::resetFault(EtherCAT::CoE::CoEManager& sdo, uint16_t slave_idx) {
    ...
    if (sw_result.has_value()) {
        ...
        if (fault_cleared) {
            ...
            TETHER_LOGW(TAG, "Slave %u: Fault NOT cleared (StatusWord=0x%04X)", slave_idx, statusword);
        }
    ...
    if (cleared) {
        TETHER_LOGI(TAG, "Slave %u: Fault appears CLEARED (0x203F external now 0)", slave_idx);
    } else {
        TETHER_LOGW(TAG, "Slave %u: Fault may persist (0x203F external=0x%04X)", slave_idx, mfr_after);
    }
    return cleared;
}
...
bool AS715NFaultHandler::handleNoSyncError(EtherCAT::CoE::CoEManager& sdo, uint16_t slave_idx, uint8_t max_attempts) {
    ...
    if (mfr == 0) {
        TETHER_LOGI(TAG, "Slave %u: No manufacturer fault present", slave_idx);
        return true;  // No fault
    }

    AS715NError err = AS715NError::parse(mfr);
    if (!err.isDCSyncError()) {
        TETHER_LOGW(TAG, "Slave %u: Error %s is not a DC sync error — cannot handle with sync recovery",
                    slave_idx, err.name);
        return false;
    }

    TETHER_LOGI(TAG, "Slave %u: Handling DC sync error %s (%s), max %u attempts",
                slave_idx, err.name, err.description, max_attempts);

    for (uint8_t attempt = 1; attempt <= max_attempts || max_attempts == 0; ++attempt) {
        TETHER_LOGI(TAG, "Slave %u: DC sync error reset attempt %u/%u...",
                    slave_idx, attempt, max_attempts);
        ...
        if (resetFault(sdo, slave_idx)) {
            ...
            if (new_mfr == 0) {
                TETHER_LOGI(TAG, "Slave %u: DC sync error cleared on attempt %u", slave_idx, attempt);
                return true;
            }
            TETHER_LOGW(TAG, "Slave %u: Fault reset succeeded but manufacturer error still %u",
                        slave_idx, new_mfr);
        }
    ...
    TETHER_LOGE(TAG, "Slave %u: Failed to clear DC sync error after %u attempts — CRITICAL",
                slave_idx, max_attempts);
    return false;
}
