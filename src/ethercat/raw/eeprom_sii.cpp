...
{
    // Default conservative mailbox (EtherCAT/SOEM convention):
    // SM0: Receive (MbxIn, Master→Slave) at lower address 0x1000
    // SM1: Send    (MbxOut, Slave→Master) at higher address 0x1400
    constexpr uint16_t kDefaultReceiveAddr = 0x1000;  // SM0 (Receive/MbxIn, M→S)
    constexpr uint16_t kDefaultReceiveLen = 256;
    constexpr uint16_t kDefaultSendAddr = 0x1400;     // SM1 (Send/MbxOut, S→M)
    ...
    // MASTER->SLAVE and SLAVE->MASTER directions, prefer those addresses.
    //
