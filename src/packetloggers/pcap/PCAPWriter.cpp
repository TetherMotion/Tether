/**
 * @file PCAPWriter.cpp
 * @brief Shared PCAPNG writing implementation for EtherCAT Master and Slave
 */
...
void PCAPWriter::setSectionHeader(const SectionHeaderInfo& info) {
    std::lock_guard<std::mutex> lock(mutex_);
    sectionInfo_ = info;
}

uint8_t PCAPWriter::addInterface(const InterfaceInfo& info) {
    std::lock_guard<std::mutex> lock(mutex_);
...
bool PCAPWriter::logPacket(const uint8_t* data, size_t length,
...
void PCAPWriter::setBufferSize(size_t bytes) {
    std::lock_guard<std::mutex> lock(mutex_);
    bufferSize_ = bytes;
    writeBuffer_.reserve(bufferSize_);
}

size_t PCAPWriter::getBufferUsage() const {
    std::lock_guard<std::mutex> lock(mutex_);
...
void PCAPWriter::writeSectionHeaderBlock() {
...
void PCAPWriter::writeInterfaceDescriptionBlock(const InterfaceInfo& info) {
    std::vector<uint8_t> content;
    content.reserve(256);
...
void PCAPWriter::writeEnhancedPacketBlock(const uint8_t* data, size_t length,
                                            const PacketMetadata& metadata) {
    std::vector<uint8_t> content;
...
void PCAPWriter::writeBlock(uint32_t blockType, const std::vector<uint8_t>& content) {
...
void PCAPWriter::writeOption(std::vector<uint8_t>& options, uint16_t code,
...
void PCAPWriter::writeOptionString(std::vector<uint8_t>& options, uint16_t code,
                                     const std::string& str) {
    writeOption(options, code, str.data(), str.length());
}

void PCAPWriter::writeEndOfOptions(std::vector<uint8_t>& options) {
...
uint32_t PCAPWriter::padTo32(uint32_t length) {
    return (length + 3) & ~3;
}

uint64_t PCAPWriter::getCurrentTimestampNs() const {
...
std::string PCAPWriter::getRotatedFilename(uint32_t index) const {
...
// ============================================================================
// Null Writer (discards all packets)
// ============================================================================

class NullPCAPWriter : public IPCAPWriter {
public:
...
class MemoryPCAPWriter : public PCAPWriter {
public:
    explicit MemoryPCAPWriter(size_t maxBytes)
        : maxBytes_(maxBytes)
    {}
...
std::unique_ptr<IPCAPWriter> createPCAPWriter() {
    return std::make_unique<PCAPWriter>();
}

std::unique_ptr<IPCAPWriter> createPCAPWriter(const std::string& filename,
                                              const SectionHeaderInfo& sectionInfo) {
    auto logger = std::make_unique<PCAPWriter>();
    logger->setSectionHeader(sectionInfo);
    logger->open(filename);
    return logger;
}

std::unique_ptr<IPCAPWriter> createNullPCAPWriter() {
    return std::make_unique<NullPCAPWriter>();
}

std::unique_ptr<IPCAPWriter> createMemoryPCAPWriter(size_t maxBytes) {
    return std::make_unique<MemoryPCAPWriter>(maxBytes);
}
