...
double meanVal(const std::vector<double>& v) {
...
void PathRelativeFFT::detrend(std::vector<double>& signal,
                              bool removeDC, bool removeLinear) const {
    if (signal.empty()) return;

    if (config_.removeLinearTrend && removeLinear) {
    ...
    if (removeDC) {
        double m = meanVal(signal);
        for (double& v : signal) v -= m;
    }
}

void PathRelativeFFT::applyWindow(std::vector<double>& signal,
...
void PathRelativeFFT::fft(std::vector<std::complex<double>>& data) const {
    auto n = data.size();
    if (n <= 1) return;
...
std::vector<SpectralPeak> PathRelativeFFT::findPeaks(
...
ComponentSpectrum PathRelativeFFT::computeComponentSpectrum(
    ...
    // Detrend
    detrend(uniform, config_.removeDC, config_.removeLinearTrend);

    // Apply window
    applyWindow(uniform, config_.window);
    ...
    // Find peaks
    spec.peaks = findPeaks(spec.frequencies, spec.magnitudes, spec.phases,
