...
namespace {
int safe_bounds_axis(int axis) {
    return (axis >= 0 && axis < 3) ? axis : 0;
}
} // namespace
...
void Heatmap1D::rebuildBins() {
    const int boundsAxis = safe_bounds_axis(axis_);
    double range = config_.maxBounds[boundsAxis] - config_.minBounds[boundsAxis];
...
size_t Heatmap1D::positionToBin(double pos) const {
    const int boundsAxis = safe_bounds_axis(axis_);
    double normalized = (pos - config_.minBounds[boundsAxis]) / config_.resolution1D;
...
double Heatmap1D::binToPosition(size_t bin) const {
    const int boundsAxis = safe_bounds_axis(axis_);
    return config_.minBounds[boundsAxis] + (bin + 0.5) * config_.resolution1D;
}
