#include "tether/simulation/systems/mechanical/Acrobot.hpp"
#include <cmath>
#include <stdexcept>
...
StateVector Acrobot::dynamics(double /*t*/, const StateVector& s, const StateVector& u) const {
    ...
    double tau = u.empty() ? 0.0 : u[0]; // torque at elbow only
    ...
    // Acrobot: torque at elbow -> [0; tau]
    double ddth1 = (d22*(-b1*dth1 - h1 - phi1) - d12*(tau - b2*dth2 - h2 - phi2)) / det;
    double ddth2 = (-d12*(-b1*dth1 - h1 - phi1) + d11*(tau - b2*dth2 - h2 - phi2)) / det;

    return {dth1, ddth1, dth2, ddth2};
}

StateVector Acrobot::output(double /*t*/, const StateVector& s, const StateVector& /*u*/) const { return {s[0], s[2]}; }
StateVector Acrobot::defaultInitialState() const { return {M_PI, 0, 0, 0}; }

std::vector<ParamDescriptor> Acrobot::parameterDescriptors() const {
    return {{"m1","kg","Link 1 mass",1.0,0.01,10.0,0.1},{"m2","kg","Link 2 mass",1.0,0.01,10.0,0.1},
            ...
            {"lc1","m","CoM 1",0.5,0.01,5.0,0.01},{"lc2","m","CoM 2",0.5,0.01,5.0,0.01},
            {"I1","kg·m²","Inertia 1",0.083,0.001,10.0,0.001},{"I2","kg·m²","Inertia 2",0.083,0.001,10.0,0.001},
            {"g","m/s²","Gravity",9.81,0.1,20.0,0.01},{"b1","N·m·s","Damp 1",0.1,0.0,5.0,0.01},{"b2","N·m·s","Damp 2",0.1,0.0,5.0,0.01}};
}

std::vector<Preset> Acrobot::presets() const {
    return {{"Standard","Default",{{"m1",1.0},{"m2",1.0},{"l1",1.0},{"l2",1.0},{"lc1",0.5},{"lc2",0.5},{"I1",0.083},{"I2",0.083},{"g",9.81},{"b1",0.1},{"b2",0.1}}}};
}
void Acrobot::applyPreset(int i) { auto p=presets(); if(i>=0&&i<(int)p.size()) setParameters(p[i].params); }
std::vector<std::string> Acrobot::stateNames() const { return {"θ₁","dθ₁/dt","θ₂","dθ₂/dt"}; }
std::vector<std::string> Acrobot::outputNames() const { return {"θ₁","θ₂"}; }
std::vector<std::string> Acrobot::inputNames() const { return {"Elbow torque"}; }
