...
TEST(SIIMailbox, SMControlValidationNoWarning) {
    ...
    // SM0: mailbox mode + ECAT writes (master→slave, Receive/MbxIn): SM_CTRL_MODE_MAILBOX | SM_CTRL_DIR_WRITE | SM_CTRL_WATCHDOG
    uint8_t sm0_ok = static_cast<uint8_t>(0x02u | 0x04u | 0x20u); // 0x26
    // SM1: mailbox mode + ECAT reads (slave→master, Send/MbxOut): SM_CTRL_MODE_MAILBOX | SM_CTRL_DIR_READ | SM_CTRL_WATCHDOG
